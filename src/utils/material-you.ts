 ...
 * This file generates M3 color values at runtime via Google's Material Color Utilities,
 * overriding the compile-time defaults defined in styles/variables.styl.
 * When adding/removing a legacy CSS variable (e.g. --primary, --card-bg),
 * you must update BOTH this file (LEGACY_MAPPINGS) AND variables.styl.
 */
import {
	DynamicScheme,
	Hct,
...
function resolveColor(scheme: DynamicScheme, snakeKey: string): string | null {
	const camelKey = snakeToCamel(snakeKey) as keyof typeof MaterialDynamicColors;
	const fn = MaterialDynamicColors[camelKey];
	if (typeof fn !== "function") return null;
	const dynamicColor = (
		fn as () => { getArgb: (s: DynamicScheme) => number }
	).call(MaterialDynamicColors);
...
export function applyM3Theme(hue?: number): void {
	...
	console.log("[Color] applying M3 theme with hue:", currentHue);
